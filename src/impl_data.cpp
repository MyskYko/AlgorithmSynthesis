...
void impl_data::read_file(std::string file_name) {
...
void impl_data::create_selection_signal() {
  ...
  for(auto x_name: x_names) {
    for(unsigned int i = 0; i < x_candidate_names[x_name].size(); i++) {
      ...
      if(x_group[x_name] == 0) {
	selection_signal = "__from" + x_candidate_names[x_name][i] + "to" + x_name;
      }
      else {
	selection_signal = "__from" + x_candidate_names[group_symbol[x_group[x_name]]][i] + "to" + group_symbol[x_group[x_name]];	
      }
